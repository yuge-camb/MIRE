    ...
    async def check_consistency(self, current_segment: Dict, previous_segments: List[Dict]) -> ContradictionResult:
        ...
        try:
            ...
            if not previous_segments:
                logging.debug(f"No previous segments to check against for UUID: {current_segment['uuid']}")
            ...
            async with asyncio.Lock():  # Protect model inference
                with torch.no_grad():
                    for prev_segment in previous_segments:
                        ...
                        logging.info(f"📊 [Consistency] Contradiction score: {contradiction_score:.3f}")
            ...
            logging.debug(f"Consistency check complete for {current_segment['uuid']}: {detected}")
